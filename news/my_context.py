...
def items_article_footer_random(request):
    skip_slug = get_skip_slug_article(request.path)
    items_article_footer_random = Article.objects.filter(status=APP_VALUE_STATUS_ACTIVE, publish_date__lte=timezone.now()).exclude(slug=skip_slug).order_by('?')[:SETTING_ARTICLE_TOTAL_ITEM_RANDOM]
    return {'items_article_footer_random': items_article_footer_random}

def items_article_header_trending(request):
    skip_slug = get_skip_slug_article(request.path)
    items_article_header_trending = Article.objects.filter(status=APP_VALUE_STATUS_ACTIVE, publish_date__lte=timezone.now()).exclude(slug=skip_slug).order_by('?')[0]
